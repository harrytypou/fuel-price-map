...
function moveTooltip(event) {
  ...
  if (y + height > rect.height - margin) {
    y = event.clientY - rect.top - height - margin;
  }

  tooltip.style.left = `${Math.max(margin, Math.min(x, rect.width - width - margin))}px`;
  tooltip.style.top = `${Math.max(margin, Math.min(y, rect.height - height - margin))}px`;
}
